...
def selectcontrol(request):
        ...
        group_df=grouplist=ProcessReg.objects.filter(user=request.user).exclude(category='Individual').values_list('businessdefinition_a','regulation','process')
        if group_df:
...
def selectrisk(request):
        ...
        single_chart_data_list=[]#to store json data for each business activity
        group_chart_data_list=[]#to store json data for each business activity
        group_df=grouplist=ControlReg.objects.filter(user=request.user).exclude(category='Individual').values_list('businessdefinition_a','regulation','process','controlarea')
        if group_df:
...
def viewfinalmapping(request):
    ...
    if group == 'yes':   # Ig group is yes, Remove temp group name in all tables, as many balist times, for a specific user
        ...
        for x in riskobjs:
            count1=RiskReg.objects.filter(regulation=x.regulation,businessdefinition_a=x.businessdefinition_a,process=x.process,controlarea=x.controlarea,risk=x.risk, user=request.user,status='done',category='Group').count()
            if(count1<1):
                
                RiskReg(regulation=x.regulation,businessdefinition_a=x.businessdefinition_a,process=x.process,controlarea=x.controlarea,risk=x.risk, user=request.user,status='done',category='Group').save()
                ControlReg(regulation=x.regulation,businessdefinition_a=x.businessdefinition_a,process=x.process,controlarea=x.controlarea, user=request.user,status='done',category='Group').save()
                ProcessReg(regulation=x.regulation,businessdefinition_a=x.businessdefinition_a,process=x.process, user=request.user,status='done',category='Group').save()
                BAReg(regulation=x.regulation,businessdefinition_a=x.businessdefinition_a,user=request.user,status='done',category='Group').save()
                for ba in balist:
    ...
    # RiskReg.objects.filter(user=request.user,category=name).delete()
        finalGraph(request)   
        print(finalGraph(request).group_chart_data_list)
        ...
        RequestConfig(request).configure(table) 
        context = {'table': table,'group_data':finalGraph(request).group_chart_data_list, 'single_data': finalGraph(request).single_chart_data_list,'individuals':individuals, 'groups':groups,'user' : current_user}
...
def finalGraph(request):
    ...
    balist=request.session['balist']
    group=request.session['group']
    name=request.session['groupname']#to store json data for each business activity
    #graph for group of business activities
