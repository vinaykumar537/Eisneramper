       ...
       if form.is_valid():
           ...
           if regulations: # Storing regualtion & BA Group Mapping in BAReg
                   for regulation in regulations:
                       count=BAReg.objects.filter(regulation=regulation, businessdefinition_a =name , user=user,category='Group').count()
                       if (count<1):
                          BAReg(regulation=regulation, businessdefinition_a = name, user=user,category='Group').save()
...
def secondpage(request):
        ...
        groups=[]
        individuals=[]
        grouplist=BAReg.objects.filter(user=request.user, category='Group').values_list('businessdefinition_a')
        ...
        group_df=BAReg.objects.filter(user=request.user, category='Group').values_list('businessdefinition_a','regulation')
        if group_df:
...
def selectcontrol(request):
        ...
        groups=[]
        individuals=[]
        grouplist=ProcessReg.objects.filter(user=request.user, category='Group').values_list('businessdefinition_a')
        ...
        group_df=ProcessReg.objects.filter(user=request.user, category='Group').values_list('businessdefinition_a','regulation','process')
        if group_df:
        ...
        if single_df:
            df=pd.DataFrame(list(single_df),columns=['Business_Definition','Regulation','Policy'])
            df2=df.Policy.apply(pd.Series).merge(df, right_index = True, left_index = True).drop(["Policy"], axis = 1).melt(id_vars = ['Business_Definition','Regulation'], value_name = "Policy").drop("variable", axis = 1).dropna()
            n=len(individuals)#to store number of business activities for current user
...
def selectrisk(request):
        ...
        groups=[]
        individuals=[]
        grouplist=ControlReg.objects.filter(user=request.user, category='Group').values_list('businessdefinition_a')
        ...
        single_chart_data_list=[]#to store json data for each business activity
        group_chart_data_list=[]#to store json data for each business activity
        group_df=ControlReg.objects.filter(user=request.user, category='Group').values_list('businessdefinition_a','regulation','process','controlarea')
        if group_df:
...
def select_risk(request, pk):
    ...
    if request.method == 'POST':
        ...
        if formset.is_valid():
            ...
            for form in formset:
                f = form.cleaned_data   # Extracting the data from Risk Form Set
                risk= f.get('risk')
                ...
                controlobjective=item.controlobjective, risk=risk, comment=comment, user=user,category=item.category).save()  # Saving the Selected data into Risk Reg Table (Model)
...
def showfinalmapping(request):
    ...
    groups=[]
    individuals=[]
    grouplist=RiskReg.objects.filter(user=request.user, category='Group').values_list('businessdefinition_a')
    ...
    single_chart_data_list=[]#to store json data for each business activity
    ## Grpah for group
    group_df=RiskReg.objects.filter(user=request.user, category='Group').values_list('businessdefinition_a','regulation','process','controlarea','risk')
    if group_df:
...
def viewfinalmapping(request):
    ...
    #     set1=RiskReg.objects.filter(user=request.user,category='Group')
    #     BAReg.objects.filter(user=request.user, category='Group').delete()
    #     ControlReg.objects.filter(user=request.user, category='Group').delete()
    #     ProcessReg.objects.filter(user=request.user, category='Group').delete()
    #     for x in set1:
    #         for ba in balist:
    ...
    #     RiskReg.objects.filter(user=request.user,category='Group').delete()
...
def resetMapping(request):
    Business.objects.filter(user=request.user).delete()
    BAReg.objects.filter(user=request.user).delete()
    ProcessReg.objects.filter(user=request.user).delete()
    ControlReg.objects.filter(user=request.user).delete()
    RiskReg.objects.filter(user=request.user).delete()
    # Maptable.objects.filter(user=request.user).delete()
    return redirect('selectbusiness')
...
def reset_mapping_bareg(request):
   ...
   for ba in balist:
       baobject=BAReg.objects.filter(businessdefinition_a = ba).delete()
   return redirect('selectbusiness')
...
def reset_mapping_bareg_group(request):
   ...
   name=request.session['groupname']  
   if group=="yes":
        BAReg.objects.filter(businessdefinition_a = name).delete() 
        for ba in balist:
            ProcessReg.objects.filter(businessdefinition_a = name).delete()
        return redirect('selectbusiness')
   else:
        Business.objects.filter(businessactivity__in = balist).delete()
        for ba in balist:
...
def bagroupall(request):
   ...
   if request.method == 'POST':
       ...
       if form.is_valid():
           ...
           if regulations: # Storing regualtion & BA Group Mapping in BAReg
                   for regulation in regulations:
                       count=BAReg.objects.filter(regulation=regulation, businessdefinition_a =name , user=user,category=name).count()
                       if (count<1):
                          BAReg(regulation=regulation, businessdefinition_a = name, user=user,category=name).save()
...
def secondpage(request):
        ...
        name=request.session['groupname']
        grouplist=BAReg.objects.filter(user=request.user,category__in=['Group',name]).values_list('businessdefinition_a')
        print(grouplist)
        individualist=BAReg.objects.filter(user=request.user,category='Individual').values_list('businessdefinition_a')
        grouplist=list(grouplist)
        individualist=list(individualist)
        ...
        y =BAReg.objects.filter(user=current_user).exclude(category='Group')
        print(y)
        

        table = PersonTable(BAReg.objects.filter(user=current_user).exclude(category ='Group'))
        print(table)
        RequestConfig(request).configure(table)
        ...
        if single_df:
            ...
            individuals.reverse()
            single_chart_data_list.reverse()
        group_df=BAReg.objects.filter(user=request.user).exclude(category='Individual').values_list('businessdefinition_a','regulation')
        if group_df:
            df2=pd.DataFrame(list(group_df),columns=['Business_Definition','Regulation'])
            # df2=df.Regulation.apply(pd.Series).merge(df, right_index = True, left_index = True).drop(["Regulation"], axis = 1).melt(id_vars = ['Business_Definition'], value_name = "Regulation").drop("variable", axis = 1).dropna()
            n=len(groups)#to store number of business activities for current user
...
def thirdpage(request):
        ...
        if count == done_count: 
           ...
           button_enable = False
        table = ControlTable(ProcessReg.objects.filter(user=current_user).exclude(category='Group'))  # Configuring the  Control Table with Process Reg Data
        RequestConfig(request).configure(table)
        context = {'table': table, 'flag' : button_enable}
        return render(request,'Process_2.html/',context)
...
def fourthpage(request):
        ...
        if count == done_count:
           ...
           button_enable = False
        table = RiskTable(ControlReg.objects.filter(user=current_user).exclude(category='Group'))  # Configuring the Risk Table with Control Reg Data
        RequestConfig(request).configure(table)
        context = {'table': table, 'flag' : button_enable}
        return render(request,'fourthpage.html/',context)
...
def selectcontrol(request):
        ...
        groups=[]
        individuals=[]
        grouplist=ProcessReg.objects.filter(user=request.user,category__in=['Group', name]).values_list('businessdefinition_a')
        individualist=ProcessReg.objects.filter(user=request.user,category='Individual').values_list('businessdefinition_a')
        grouplist=list(grouplist)
        individualist=list(individualist)
        for group in grouplist:
        ...
        group_chart_data_list=[] #to store json data for each business activity
        #graph for group of business activities
        group_df=ProcessReg.objects.filter(user=request.user).exclude(category='Individual').values_list('businessdefinition_a','regulation','process')
        if group_df:
            df2=pd.DataFrame(list(group_df),columns=['Business_Definition','Regulation','Policy'])
            # df2=df.Policy.apply(pd.Series).merge(df, right_index = True, left_index = True).drop(["Policy"], axis = 1).melt(id_vars = ['Business_Definition','Regulation'], value_name = "Policy").drop("variable", axis = 1).dropna()
            b_color="#4d88ff"
...
def selectrisk(request):
        ...
        if count == done_count:
           ...
           button_enable = False
        table = RiskTable(ControlReg.objects.filter(user=current_user).exclude(category='Group'))  # Configuring the Risk Table with Control Reg Table Data
        RequestConfig(request).configure(table)
        group=request.session['group']
         #graph for group of business activities
        groups=[]
        individuals=[]
        grouplist=ControlReg.objects.filter(user=request.user, category__in=['Group', name]).values_list('businessdefinition_a')
        individualist=ControlReg.objects.filter(user=request.user,category='Individual').values_list('businessdefinition_a')
        grouplist=list(grouplist)
        individualist=list(individualist)
        for group in grouplist:
        ...
        single_chart_data_list=[]#to store json data for each business activity
        group_chart_data_list=[]#to store json data for each business activity
        group_df=ControlReg.objects.filter(user=request.user).exclude(category='Individual').values_list('businessdefinition_a','regulation','process','controlarea')
        if group_df:
            df2=pd.DataFrame(list(group_df),columns=['Business_Definition','Regulation','Policy','Control'])   
            # df2=df.Control.apply(pd.Series).merge(df, right_index = True, left_index = True).drop(["Control"], axis = 1).melt(id_vars = ['Business_Definition','Regulation','Policy'], value_name = "Control").drop("variable", axis = 1).dropna()
            b_color="#4d88ff"
...
def showfinalmapping(request):
    ...
    if count == done_count:
       ...
       button_enable = False
    table = FinalTable(RiskReg.objects.filter(user=current_user).exclude(category='Group')) # Configuring the Risk Reg Data in Final Table
    RequestConfig(request).configure(table)
    groups=[]
    individuals=[]
    grouplist=RiskReg.objects.filter(user=request.user, category__in=['Group', name]).values_list('businessdefinition_a')
    individualist=RiskReg.objects.filter(user=request.user,category='Individual').values_list('businessdefinition_a')
    grouplist=list(grouplist)
    individualist=list(individualist)
    for group in grouplist:
    ...
    single_chart_data_list=[]#to store json data for each business activity
    ## Grpah for group
    group_df=RiskReg.objects.filter(user=request.user).exclude(category='Individual').values_list('businessdefinition_a','regulation','process','controlarea','risk')
    if group_df:
        df2=pd.DataFrame(list(group_df),columns=['Business_Definition','Regulation','Policy','Control','Risk'])
        # df2=df.Risk.apply(pd.Series).merge(df, right_index = True, left_index = True).drop(["Risk"], axis = 1).melt(id_vars = ['Business_Definition','Regulation','Policy','Control'], value_name = "Risk").drop("variable", axis = 1).dropna()
        b_color="#4d88ff"
...
def viewfinalmapping(request):
    ...
    if group == 'yes':   # Ig group is yes, Remove temp group name in all tables, as many balist times, for a specific user
        riskobjs=RiskReg.objects.filter(user=request.user,businessdefinition_a=name)
        print(RiskReg.objects.filter(businessdefinition_a = name))
        # BAReg.objects.filter(user=request.user, category=name).delete()
        # ControlReg.objects.filter(user=request.user, category=name).delete()
        # ProcessReg.objects.filter(user=request.user, category=name).delete()
        for x in riskobjs:
            count=RiskReg.objects.filter(regulation=x.regulation,businessdefinition_a=x.businessdefinition_a,process=x.process,controlarea=x.controlarea,risk=x.risk, user=request.user,status='done',category='Group').count()
            print(count)
            if(count<1):
                
                RiskReg.objects.filter(businessdefinition_a = name).update(category = 'Group')
                ControlReg.objects.filter(businessdefinition_a = name).update(category = 'Group')                
                ProcessReg.objects.filter(businessdefinition_a = name).update(category = 'Group')
                BAReg.objects.filter(businessdefinition_a = name).update(category = 'Group')
                ...
                for ba in balist:
                    ...
                    if(count1<1):   # To avoid duplicates
                        ...
                        ProcessReg(regulation=x.regulation,businessdefinition_a=ba,process=x.process, user=request.user,status='done',category=name).save()
                        BAReg(regulation=x.regulation,businessdefinition_a=ba,user=request.user,status='done',category=name).save()
    # RiskReg.objects.filter(user=request.user,category=name).delete()
        finalGraph(request)   
       

        finaaaal = finalGraph(request)

        
    
        table = FinalviewTable(RiskReg.objects.filter(user=current_user).exclude(category='Group'))  # Configuring the Final view Table with Risk Reg Data
        RequestConfig(request).configure(table) 
        context = {'table': table,'group_data':finaaaal[0], 'single_data': finaaaal[1],'individuals':individuals, 'groups':groups,'user' : current_user}
        return render(request,'finalview.html/',context)
...
def finalGraph(request):
    groups=[]
    individuals=[]
    group_chart_data_list = []
    single_chart_data_list = []
    grouplist=RiskReg.objects.filter(user=request.user,category='Group').values_list('businessdefinition_a')
    individualist=RiskReg.objects.filter(user=request.user,category='Individual').values_list('businessdefinition_a')
    grouplist=list(grouplist)
    individualist=list(individualist)
    ...
    groups=list(set(groups))
    individuals=list(set(individuals))
    #to store json data for each business activity
    #graph for group of business activities
    group_df=RiskReg.objects.filter(user=request.user, category='Group').values_list('businessdefinition_a','regulation','process','controlarea','risk')
    df2=pd.DataFrame(list(group_df),columns=['Business_Definition','Regulation','Policy','Control','Risk'])
    n=len(groups)#to store number of business activities for current user
    ...
    return [group_chart_data_list,single_chart_data_list]
...
@login_required
def existingMapping(request):
   table = FinalviewTable(RiskReg.objects.filter(user=request.user).exclude(category = 'Group'))  # Configuring the RiskReg Table data in Final View Table
   RequestConfig(request).configure(table)
   context ={'table':table,'user':request.user}
   return render(request, 'final.html', context)
...
def viewfinalmapping(request):
    ...
    if group == 'yes':   # Ig group is yes, Remove temp group name in all tables, as many balist times, for a specific user
        set1=RiskReg.objects.filter(user=request.user,category=name)
        # BAReg.objects.filter(user=request.user, category=name).delete()
        # ControlReg.objects.filter(user=request.user, category=name).delete()
        # ProcessReg.objects.filter(user=request.user, category=name).delete()
        for x in set1:
